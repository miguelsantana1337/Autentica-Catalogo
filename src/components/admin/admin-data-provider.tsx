...
function productRecord(product: Product) {
  return { id: product.id, slug: product.slug, name: product.name, category_id: product.categoryId, brand: product.brand, price: product.price, compare_at: product.compareAt, cashback: product.cashback, cost_price: product.costPrice, stock: product.stock, min_stock: product.minStock, badge: product.badge, accent: product.accent, description: product.description, tagline: product.tagline, need: product.need, audiences: product.audiences, indication: product.indication, result: product.result, sku: product.sku, rating: product.rating, reviews: product.reviews, featured: product.featured, active: product.active, order_index: product.order, image_url: product.imageUrl, image_urls: product.imageUrls, product_type: product.productType, regulatory_status: product.regulatoryStatus, active_ingredient: product.activeIngredient, anvisa_registration: product.anvisaRegistration, presentation: product.presentation, regulatory_warning: product.regulatoryWarning, pharmacist_reviewed: product.pharmacistReviewed };
}
...
export function AdminDataProvider({ initialData, currentUser, children }: { initialData: StoreData; currentUser: AdminDataContextValue["currentUser"]; children: ReactNode }) {
  ...
  const syncStorefront = store.importData;
  ...
  useEffect(() => {
    ...
    syncStorefront({
      tenant: {
        ...
        storefrontPath: data.tenant.storefrontPath,
      },
      settings: data.settings,
      products: data.products
        .filter(isProductVisibleInCatalog)
        .map((product) => sanitizeProductForStorefront(product)),
  ...
  const updateOrderStatus = useCallback(async (id: string, status: OrderStatus) => {
    await commitMutation((current) => {
      ...
      const rawGeneratedLogs = changedOrder && changedOrder.status !== previousOrder?.status
        ? createMessageLogs(changedOrder, current.messageAutomations)
        : [];
      const executionTimestamp = new Date().toISOString();
      const generatedRuns: AutomationRun[] = rawGeneratedLogs.map((log) => {
        const automation = current.messageAutomations.find((item) => item.id === log.automationId);
        const runId = crypto.randomUUID();
        const failed = !log.recipient;
        log.runId = runId;
        log.status = failed ? "failed" : "simulated";
        log.errorMessage = failed ? "Destinatário não disponível" : "";
        return {
          id: runId,
          automationId: log.automationId,
          automationName: log.automationName,
          triggerType: automation?.triggerType ?? "order_status",
          triggerEvent: { orderId: log.orderId, orderCode: log.orderCode, status },
          status: failed ? "failed" : "simulated",
          attempt: 1,
          maxAttempts: (automation?.maxRetries ?? 0) + 1,
          output: { channel: log.channel, recipient: log.recipient, subject: log.subject, message: log.message },
          errorMessage: log.errorMessage,
          nextRetryAt: failed && (automation?.maxRetries ?? 0) > 0
            ? new Date(Date.now() + (automation?.retryDelayMinutes ?? 0) * 60_000).toISOString()
            : "",
          startedAt: executionTimestamp,
          finishedAt: executionTimestamp,
          actorEmail: currentUser.email,
          createdAt: executionTimestamp,
        };
      });
      const generatedAutomationIds = new Set(generatedRuns.map((run) => run.automationId));
      const failedAutomationIds = new Set(generatedRuns.filter((run) => run.status === "failed").map((run) => run.automationId));
      ...
      return {
        ...
        orders: nextOrders,
        couponRedemptions,
        automationRuns: [...generatedRuns, ...current.automationRuns],
        messageLogs: [...rawGeneratedLogs, ...current.messageLogs],
        messageAutomations: current.messageAutomations.map((automation) => generatedAutomationIds.has(automation.id)
          ? {
              ...automation,
              runCount: automation.runCount + 1,
              failureCount: automation.failureCount + (failedAutomationIds.has(automation.id) ? 1 : 0),
            }
          : automation),
      };
    }, async (next, previous) => {
      ...
      if (!error) {
        await Promise.all([refreshCashbackEntries(), refreshMarketingStudio()]);
        return;
      }
      if (error.code !== "PGRST202" && error.code !== "42883") throw new Error(error.message);
  ...
  }, [commitMutation, currentUser.email, persist, refreshCashbackEntries, refreshMarketingStudio, supabase, update]);
