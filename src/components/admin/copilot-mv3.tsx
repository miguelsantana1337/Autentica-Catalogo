...
export function CopilotMV3() {
  const { data, currentUser } = useAdminData();
  const pathname = usePathname();
  const supabase = useMemo(() => createClient(), []);
  const storageKey = `mv3-catalog:copilot:${data.tenant.id}:${currentUser.id}`;
  ...
  return <>
    <button className={`copilot-launcher ${open ? "is-open" : ""}`} type="button" onClick={() => setOpen((current) => !current)} aria-label={open ? "Fechar Copiloto MV3" : "Abrir Copiloto MV3"} aria-expanded={open}>
      {open ? <PanelRightClose /> : <Sparkles />}<span><strong>Copiloto MV3</strong><small>Somente leitura</small></span><kbd>⌘ J</kbd>
    </button>
    {open && <button className="copilot-backdrop" onClick={() => setOpen(false)} aria-label="Fechar Copiloto" />}
    <aside className={`copilot-panel ${open ? "open" : ""}`} aria-hidden={!open} aria-label="Copiloto MV3">
      <header><div className="copilot-avatar"><Bot /><i /></div><div><span><Sparkles /> COPILOTO MV3</span><strong>Como posso ajudar?</strong><small><ShieldCheck /> Modo local seguro · somente leitura</small></div><button onClick={() => setOpen(false)} aria-label="Fechar"><X /></button></header>
