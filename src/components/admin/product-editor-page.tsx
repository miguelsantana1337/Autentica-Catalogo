...
const stepFields: Array<Array<keyof Product>> = [
  ["name", "sku", "categoryId", "brand", "description", "tagline", "need", "audiences", "indication", "result"],
  ["imageUrl", "imageUrls"],
...
export function ProductEditorPage({ productId }: { productId?: string }) {
  ...
  const draftKey = `mv3-catalog:product-draft:${productId ?? "new"}`;
          ...
          {step === 0 && (
            <section className="product-editor-section" aria-labelledby="product-information-title">
              ...
              <div className="product-editor-fields">
                ...
                <label className="full">Chamada curta<input value={form.tagline} onChange={(event) => field("tagline", event.target.value)} placeholder="Ex.: Reconstrução para fibra quebradiça." /></label>
                <label>Necessidade principal<input value={form.need} aria-invalid={Boolean(fieldErrors.need)} onChange={(event) => field("need", event.target.value)} placeholder="Ex.: Cronograma capilar" />{issue("need")}</label>
                <fieldset className="full"><legend>Públicos</legend><div className="product-publish-options"><label><input type="checkbox" checked={form.audiences.includes("b2b")} onChange={(event) => field("audiences", event.target.checked ? [...new Set([...form.audiences, "b2b" as const])] : form.audiences.filter((item) => item !== "b2b"))} /><span><strong>Profissionais e salões</strong></span></label><label><input type="checkbox" checked={form.audiences.includes("resale")} onChange={(event) => field("audiences", event.target.checked ? [...new Set([...form.audiences, "resale" as const])] : form.audiences.filter((item) => item !== "resale"))} /><span><strong>Revenda e distribuição</strong></span></label><label><input type="checkbox" checked={form.audiences.includes("b2c")} onChange={(event) => field("audiences", event.target.checked ? [...new Set([...form.audiences, "b2c" as const])] : form.audiences.filter((item) => item !== "b2c"))} /><span><strong>Cliente final</strong></span></label></div>{issue("audiences")}</fieldset>
                <label className="full">Ativos principais<input value={form.activeIngredient} onChange={(event) => field("activeIngredient", event.target.value)} /></label>
                <label className="full">Indicação<textarea value={form.indication} onChange={(event) => field("indication", event.target.value)} rows={3} /></label>
                <label className="full">Resultado esperado<textarea value={form.result} onChange={(event) => field("result", event.target.value)} rows={3} /></label>
              </div>
            </section>
          )}
