  ...
  return (
    <article className="product-card" data-testid={`product-${product.slug}`}>
      ...
      <div className="product-card-body">
        <div className="product-meta"><span>{product.category}</span><small className={`stock-${stock.tone}`}>{data.settings.showPrices ? stock.label : "Sob consulta"}</small></div>
        <Link href={detailHref}><h3>{product.name}</h3></Link>
        <p>{product.description}</p>
        {data.settings.showPrices && product.cashback > 0 && <div className="product-cashback-badge">Ganhe {formatMoney(product.cashback)} de cashback</div>}
        {product.reviews > 0 && <div className="rating" aria-label={`${product.rating} de 5 estrelas`}>★★★★★ <span>{product.rating} ({product.reviews})</span></div>}
        <div className="product-bottom">
          {data.settings.showPrices ? <div className="price-stack">
          ...
          </div> : <div className="price-stack quote-price"><strong>Solicite um orçamento</strong><small>Condições pelo WhatsApp</small></div>}
          {cartEligible ? <button
              ...
              aria-label={`Adicionar ${product.name} à ${data.settings.showPrices ? "sacola" : "solicitação"}`}
            ><Plus /></button>
