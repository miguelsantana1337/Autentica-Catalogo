...
export function StoreHeader() {
  ...
  function submitSearch(event: React.FormEvent) {
    ...
    if (pathname === (data.tenant.storefrontPath || "/")) {
      window.dispatchEvent(new CustomEvent("mv3-search", { detail: query }));
    ...
    <>
      <div className="announcement">
        <div className="container announcement-inner">
          ...
          <Link href={storeHref("/#catalogo")}>Explorar produtos →</Link>
        </div>
      </div>
        ...
        <div className="container header-inner">
          ...
          <div className="header-actions">
            ...
            <button
              ...
              aria-label={`Abrir ${data.settings.showPrices ? "carrinho" : "seleção"} com ${itemCount} itens`}
            >
              <ShoppingCart />
              <span>{data.settings.showPrices ? "Carrinho" : "Seleção"}</span>
              <b>{itemCount}</b>
            </button>
            <button
