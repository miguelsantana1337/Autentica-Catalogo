...
describe("armazenamento sensível no navegador", () => {
  ...
  it("remove tokens legados do Supabase mantidos no localStorage", () => {
    ...
    window.localStorage.setItem("mv3-catalog:admin-theme", "dark");
    ...
    expect(window.localStorage.getItem("mv3-catalog:admin-theme")).toBe("dark");
  });

  it("limpa rascunhos e dados administrativos ao sair", () => {
    window.sessionStorage.setItem("tenant:store-data:v1", "{}");
    window.sessionStorage.setItem("mv3-catalog:product-draft:new", "{}");
    window.localStorage.setItem("mv3-catalog:admin-sidebar", "collapsed");

    clearAdminSensitiveBrowserStorage();

    expect(window.sessionStorage.getItem("tenant:store-data:v1")).toBeNull();
    expect(window.sessionStorage.getItem("mv3-catalog:product-draft:new")).toBeNull();
    expect(window.localStorage.getItem("mv3-catalog:admin-sidebar")).toBe("collapsed");
  });
});
