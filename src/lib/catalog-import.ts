...
AUT-035;Produto exemplo;Finalização;Autêntica Professional;0;0;0;0;20;5;Descrição completa do produto;nao;nao;;nao_medicamento;pendente;;;;;nao`;
...
export function parseProductImport(text: string, existing: Product[], categories: Category[]) {
  ...
  records.forEach((record, index) => {
    ...
    const product: Product = {
      ...
      brand: record.marca?.trim() || current?.brand || "Autêntica Professional",
      price,
      ...
      need: current?.need ?? category?.name ?? "Catálogo",
      audiences: current?.audiences ?? ["b2b", "b2c", "resale"],
