...
const managedImageUrl = z.string().refine(
  (value) => !value || /^(https?:\/\/|data:image\/|\/)/i.test(value),
  "Use uma URL de imagem válida.",
);
...
export const checkoutCustomerSchema = z.object({
  ...
  profile: z.enum(["Profissional ou salão", "Revendedor ou distribuidor", "Cliente final"]),
...
export const productSchema = z.object({
  ...
  need: z.string().trim().min(2, "Informe a necessidade principal."),
  audiences: z.array(z.enum(["b2b", "b2c", "resale"])).min(1, "Selecione ao menos um público."),
  indication: z.string().trim().max(600),
  result: z.string().trim().max(600),
...
const optionalImageUrl = managedImageUrl;
...
export const settingsSchema = z.object({
  ...
  logoUrl: managedImageUrl,
  faviconUrl: managedImageUrl,
