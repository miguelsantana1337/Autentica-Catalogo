...
function mapProduct(row: Row, categories: Category[]): Product {
  ...
  return {
    ...
    need: str(row.need),
    audiences: stringList(row.audiences) as Product["audiences"],
...
function mapStorefrontProduct(row: Row, categories: Category[]): StorefrontProduct {
  ...
  return {
    ...
    tagline: str(row.tagline),
    need: str(row.need),
    audiences: stringList(row.audiences) as StorefrontProduct["audiences"],
    indication: str(row.indication),
    result: str(row.result),
    rating: num(row.rating),
    reviews: num(row.reviews),
...
function mapSettings(row: Row, fallback: StoreSettings): StoreSettings {
  return {
    ...
    showPrices: row.show_prices === undefined ? fallback.showPrices : Boolean(row.show_prices),
...
export async function getStoreData(options: AdminStoreDataOptions | PublicStoreDataOptions = {}): Promise<StoreData | StorefrontData> {
  ...
  const queries = await Promise.all([
    ...
    options.admin
      ...
      : scopeTenant(supabase.from("storefront_products").select("tenant_id, id, slug, name, category_id, brand, price, compare_at, cashback, badge, accent, description, tagline, need, audiences, indication, result, rating, reviews, featured, active, order_index, image_url, image_urls, product_type, regulatory_status, active_ingredient, anvisa_registration, presentation, regulatory_warning, pharmacist_reviewed, availability, purchase_limit"), tenantId).order("order_index"),
