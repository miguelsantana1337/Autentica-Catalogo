...
describe("preferências do painel", () => {
  ...
  it("persiste preferências separadamente por usuário", () => {
    ...
    writeAdminPreferences("cliente-demo", { favoriteHrefs: ["/admin/products"], tableDensity: "comfortable", productViews: [] });

    expect(readAdminPreferences("miguel").favoriteHrefs).toEqual(["/admin/data"]);
    expect(readAdminPreferences("cliente-demo").favoriteHrefs).toEqual(["/admin/products"]);
