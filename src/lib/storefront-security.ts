...
export function guardStorefrontRequest(request: Request, maxBytes = 24_000) {
  ...
  const requestProtocol = forwardedProtocol || url.protocol.replace(":", "");
  const allowedOrigins = new Set([url.origin]);
  if (requestHost) allowedOrigins.add(`${requestProtocol}://${requestHost}`);
  if (forwardedHost) allowedOrigins.add(`${requestProtocol}://${forwardedHost}`);
