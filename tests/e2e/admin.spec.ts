...
test("recolhe o menu lateral e mantém os indicadores sem sobreposição", async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => window.localStorage.getItem("mv3-catalog:admin-sidebar"))).toBe("collapsed");
  ...
  await expect.poll(() => page.evaluate(() => window.localStorage.getItem("mv3-catalog:admin-sidebar"))).toBe("expanded");
});
...
test("autentica no modo local e cadastra um produto", async ({ page }) => {
  ...
  await page.locator('input[type="file"]').first().setInputFiles([
    path.join(process.cwd(), "public", "demo-products", "produto-demo-cover.png"),
    path.join(process.cwd(), "public", "demo-products", "produto-demo-side.png"),
  ]);
...
test("configura mensagem automatica e registra o disparo", async ({ page }) => {
  await login(page);
  await openSection(page, "Marketing Studio");
  ...
  await automationModal.getByLabel("Evento").selectOption("Preparando");
  await automationModal.getByRole("textbox", { name: "Mensagem", exact: true }).fill("Olá, {{cliente}}! O pedido {{pedido}} está sendo preparado.");
  await automationModal.getByRole("combobox", { name: "Status", exact: true }).selectOption("active");
  ...
  await openSection(page, "Marketing Studio");
  await page.getByRole("button", { name: "Automações", exact: true }).click();
  await expect(page.getByRole("cell", { name: "Aviso de preparação", exact: true })).toBeVisible();
...
test("gerencia CRM, limite de cupom, frete e estoque por planilha", async ({ page }) => {
  ...
  const skuText = await ((page.viewportSize()?.width ?? 1280) < 760
    ? page.locator(".admin-mobile-cards .admin-product-cell > div:last-child > small").first()
    : page.locator(".admin-products-desktop .admin-product-cell > div:last-child > small").first()).textContent();
  ...
  await expect.poll(() => page.evaluate(() => {
    const key = Object.keys(window.sessionStorage).find((item) => item.endsWith(":store-data:v1"));
    if (!key) return 0;
    return JSON.parse(window.sessionStorage.getItem(key) || "{}").settings?.freeShippingThreshold || 0;
  })).toBe(777);
