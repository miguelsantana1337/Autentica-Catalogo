...
Atualizado em 18/07/2026. Esta rotina cobre os dados comerciais de cada tenant do MV3 Catalog e as mídias dos produtos, banners e páginas.
...
Por padrão, o pacote é gravado em `~/Documents/MV3 Catalog Backups` e a chave separada em `~/Library/Application Support/MV3 Catalog/backup.key`. Os dois arquivos usam permissão exclusiva do usuário. Também é possível fornecer:

- `MV3_ENV_FILE`: arquivo de ambiente específico.
- `MV3_BACKUP_DIR`: destino alternativo.
- `MV3_BACKUP_KEY`: chave AES de 32 bytes em Base64 mantida em um cofre.
- `MV3_BACKUP_ACTOR`: identificação do operador registrada no histórico.
...
```bash
pnpm backup:verify -- "/caminho/arquivo.mv3backup"
```
...
3. Confirme que o pacote pertence ao tenant `mv3-catalog`.
...
```bash
MV3_RESTORE_CONFIRM=mv3-catalog pnpm backup:restore -- "/caminho/arquivo.mv3backup"
```
