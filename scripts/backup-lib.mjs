...
export const backupFormat = "mv3-catalog-encrypted-backup";
...
export function loadLocalEnvironment(workdir = process.cwd()) {
  const environmentFile = process.env.MV3_ENV_FILE?.trim()
    ? resolve(process.env.MV3_ENV_FILE)
...
export function requireBackupConfiguration() {
  ...
  const tenantSlug = process.env.NEXT_PUBLIC_CLIENT_ID?.trim() || "mv3-catalog";
...
export function resolveBackupDirectory() {
  const configured = process.env.MV3_BACKUP_DIR?.trim();
  const directory = resolve(configured || join(homedir(), "Documents", "MV3 Catalog Backups"));
...
export function resolveBackupKey() {
  const configured = process.env.MV3_BACKUP_KEY?.trim();
  if (configured) {
    const decoded = Buffer.from(configured, "base64");
    if (decoded.length !== 32) throw new Error("MV3_BACKUP_KEY deve conter 32 bytes em Base64.");
    return { key: decoded, source: "environment" };
  }

  const keyPath = join(homedir(), "Library", "Application Support", "MV3 Catalog", "backup.key");
