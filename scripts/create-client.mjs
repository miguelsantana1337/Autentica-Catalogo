...
Opcionais:
  ...
  --keep-catalog      Mantém o catálogo da MV3 Catalog em vez do genérico
`;
}
...
const platformReplacements = new Map([
  ['"mv3-catalog"', JSON.stringify(slug)],
  ['"MV3 Catalog"', JSON.stringify(name)],
  ['"MV3"', JSON.stringify(prefix)],
  ['"/admin-brand.png"', JSON.stringify("/client-brand.svg")],
  ['"/favicon.svg"', JSON.stringify("/client-brand.svg")],
  ['"5500000000000"', JSON.stringify(whatsapp)],
  ['"contato@mv3catalog.com.br"', JSON.stringify(email)],
  ['"admin@mv3catalog.demo"', JSON.stringify(adminEmail)],
]);
...
const demoCredentialsPath = path.join(destination, "src/lib/supabase/demo-credentials.ts");
let demoCredentialsSource = await readFile(demoCredentialsPath, "utf8");
demoCredentialsSource = demoCredentialsSource.replaceAll('"mv3-demo-123"', JSON.stringify(adminPassword));
await writeFile(demoCredentialsPath, demoCredentialsSource, "utf8");
...
initialMigration = initialMigration.replaceAll("'MV3-'", `'${prefix}-'`);
...
builderMigration = builderMigration.replaceAll("MV3 Catalog", sqlValue(name));
...
saasMigration = saasMigration
  .replaceAll("'mv3-catalog'", `'${sqlValue(slug)}'`)
  .replaceAll("'MV3 Catalog'", `'${sqlValue(name)}'`)
  .replace("default 'MV3' check (order_prefix", `default '${sqlValue(prefix)}' check (order_prefix`);
await writeFile(saasMigrationPath, saasMigration, "utf8");

const packagePath = path.join(destination, "package.json");
const packageJson = JSON.parse(await readFile(packagePath, "utf8"));
packageJson.name = `${slug}-catalog`;
...
NEXT_PUBLIC_SOCIAL_IMAGE_URL=${envValue("/client-brand.svg")}
NEXT_PUBLIC_SITE_URL=${envValue("http://localhost:3000")}
NEXT_PUBLIC_METADATA_DESCRIPTION=${envValue("Catálogo digital com pedido estruturado e atendimento pelo WhatsApp.")}
...
Instância white-label criada a partir da base MV3 Catalog.
...
ID técnico: ${slug}
Prefixo: ${prefix}
Catálogo: ${args.has("keep-catalog") ? "MV3 Catalog" : "genérico"}
